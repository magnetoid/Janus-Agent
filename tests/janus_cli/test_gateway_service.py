...
import janus_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "janus gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`janus gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `janus gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `janus gateway restart`)."""
        unit_path = tmp_path / "janus-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``JANUS_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../janus_test`` JANUS_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir JANUS_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="JANUS_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/janus_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "janus gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_janus_home_for_target_user", lambda home: "/home/alice/.janus")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("JANUS_RESTART_DRAIN_TIMEOUT", "9")
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "nohup janus gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_janus_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: janus gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup janus gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.janus.gateway"])
            ),
        )
...
class TestSystemUnitJanusHome:
    """JANUS_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'JANUS_HOME=/home/alice/.janus' in unit
        assert '/root/.janus' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: JANUS_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JANUS_HOME", "/root/.janus/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JANUS_HOME=/home/alice/.janus/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_janus_home(self, monkeypatch):
        # Custom JANUS_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JANUS_HOME", "/opt/janus-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JANUS_HOME=/opt/janus-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's JANUS_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        janus_home = str(gateway_cli.get_janus_home().resolve())
        assert f'JANUS_HOME={janus_home}' in unit


class TestJanusHomeForTargetUser:
    """Unit tests for _janus_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("JANUS_HOME", raising=False)

        result = gateway_cli._janus_home_for_target_user("/home/alice")
        assert result == "/home/alice/.janus"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._janus_home_for_target_user("/home/alice")
        assert result == "/home/alice/.janus/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JANUS_HOME", "/opt/janus")

        result = gateway_cli._janus_home_for_target_user("/home/alice")
        assert result == "/opt/janus"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "janus gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_janus_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.janus should not produce a --profile flag."""
        janus_home = tmp_path / ".janus"
        janus_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JANUS_HOME", str(janus_home))
        result = gateway_cli._profile_arg(str(janus_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.janus/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".janus" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile JANUS_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "janus"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.janus/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".janus" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".janus" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_janus_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".janus" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.janus.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".janus" / "janus-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".janus" / "janus-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/janus", str(tmp_path / "alice"))
        assert result == "/opt/janus"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".janus" / "janus-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".janus" / "janus-agent"
        ...
        monkeypatch.setenv("JANUS_HOME", str(root_home / ".janus"))
        monkeypatch.setattr(gateway_cli, "get_janus_home", lambda: root_home / ".janus")
        ...
        # WorkingDirectory is anchored at the target user's JANUS_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.janus" in unit
        assert "WorkingDirectory=/home/alice/.janus/janus-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "janus-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "janus-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'janus gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'janus gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'janus gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "janus gateway run" in out


class TestLegacyJanusUnitDetection:
    """Tests for _find_legacy_janus_units / has_legacy_janus_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``janus.service`` unit behind when the service was
    renamed to ``janus-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``janus.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``janus-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Janus Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m janus_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_janus_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "janus.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_janus_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "janus.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_janus_units() is True

    def test_detects_legacy_janus_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "janus.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_janus_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``janus-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "janus-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "janus-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "janus-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_janus_units() is False

    def test_ignores_unrelated_janus_service(self, tmp_path, monkeypatch):
        """Third-party ``janus.service`` that isn't ours stays untouched.

        If a user has some other package named ``janus`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "janus.service").write_text(
            "[Unit]\nDescription=Some Other Janus\n[Service]\n"
            "ExecStart=/opt/other-janus/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_janus_units() == []
        assert gateway_cli.has_legacy_janus_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "janus.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "janus.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m janus_cli.main gateway run
          - python path/to/janus_cli/main.py gateway run
          - janus gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m janus_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/janus/janus_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/janus gateway run --replace",
            "ExecStart=/venv/bin/python /opt/janus/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"janus.service" if i == 0 else f"janus.service"  # same name
            # Test each variant fresh
            (user_dir / "janus.service").write_text(
                f"[Unit]\nDescription=Old Janus\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_janus_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "janus.service" in out
        assert "janus gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "janus.service"
...
class TestRemoveLegacyJanusUnits:
    """Tests for remove_legacy_janus_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_janus_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_janus_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop janus.service" in c for c in cmds_joined)
        assert any("--user disable janus.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo janus gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop janus.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable janus.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "janus.service"
        system_legacy = system_dir / "janus.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (janus-gateway-coder.service)
        ...
        profile_unit = user_dir / "janus-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "janus-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_janus_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `janus gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import janus_cli.main as cli_main
        ...
        # public, just confirm that `janus gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import janus_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "janus_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_janus_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "janus_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_janus_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_janus_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_janus_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "janus-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "janus-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "janus-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``janus gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "janus-gateway")
        ...
        assert "sudo systemctl start janus-gateway" in out
        assert "sudo janus gateway uninstall --system" in out
        assert "janus gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart janus-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop janus-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``janus gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (JANUS_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_janus_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".janus"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_janus_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # JANUS_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".janus"
        monkeypatch.setattr(gateway_cli, "get_janus_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_janus_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".janus"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_janus_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_janus_home(self, tmp_path, monkeypatch):
        import re
