#!/usr/bin/env python3
"""
Janus Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Janus Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills janus-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: janus_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See janus_bootstrap.py for full rationale.
try:
    import janus_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when janus_bootstrap isn't registered in the venv
    # yet — happens during partial ``janus update`` where git-reset landed
...
os.environ["JANUS_QUIET"] = "1"  # Our own modules

import yaml

from janus_cli.fallback_config import get_fallback_chain
...
try:
    from janus_cli.pt_input_extras import (
...
from janus_cli.banner import _format_context_length, format_banner_version_label

_COMMAND_SPINNER_FRAMES = ("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")


# Load .env from ~/.janus/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from janus_constants import get_janus_home, display_janus_home
from janus_cli.browser_connect import (
...
load_janus_dotenv(janus_home=_janus_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.janus/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("JANUS_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from janus_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.janus/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If JANUS_IGNORE_USER_CONFIG=1 is set (via ``janus chat --ignore-user-config``),
    the user config at ``~/.janus/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({JANUS_HOME}/config.yaml)
    user_config_path = _janus_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("JANUS_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Janus, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Janus - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see janus_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from janus_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and JanusCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (janus_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && janus` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _JANUS_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_JANUS_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["JANUS_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.janus/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from janus_logging import setup_logging
...
try:
    from janus_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``janus --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from janus_cli.mcp_startup import wait_for_mcp_discovery
...
from janus_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from janus_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("JANUS_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    try:
        from janus_cli.mcp_startup import start_background_mcp_discovery
...
def _run_cleanup():
    ...
    try:
        from janus_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run janus -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"janus-{short_id}"
    branch_name = f"janus/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`janus_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from janus_cli.config import load_config as _load_full_config
        from janus_constants import get_janus_home as _get_janus_home
        _janus_home_maint = _get_janus_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_janus_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`janus_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``janus/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("janus-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``janus/janus-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``janus -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("janus/janus-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. JANUS_LIGHT / JANUS_TUI_LIGHT env (true/false) — explicit override
#   2. JANUS_TUI_THEME=light|dark — explicit theme
#   3. JANUS_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Janus assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("JANUS_LIGHT", "JANUS_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("JANUS_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("JANUS_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from janus_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_janus_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._janus_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Janus output it's common to display cron schedules verbatim.
    #
...
from janus_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_janus_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_janus_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._janus_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_janus_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._janus_bp_start = now
        ...
        _vt100_mod._janus_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``JanusCLI._busy_command()``.
        """
        yield self

# ASCII Art - JANUS-AGENT logo (full width, single line - requires ~95 char terminal)
JANUS_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Janus Caduceus (compact, fits in left panel)
JANUS_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ JANUS - AI Agent Framework"
        tiny_line = "⚕ JANUS"
    else:
        agent_name = _skin.get_branding("agent_name", "Janus") if _skin else "Janus"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("JANUS_FAST_STARTUP_BANNER") == "1":
        from janus_cli import __release_date__ as _release_date
        from janus_cli import __version__ as _version

        version_line = f"Janus Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from janus_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.janus/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.janus/config.yaml on first use)
...
class JanusCLI:
    """
    Interactive CLI for the Janus Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: JANUS_MAX_TOKENS)
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from janus_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("JANUS_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("JANUS_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("JANUS_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `janus chat --ignore-rules` in janus_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("JANUS_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("JANUS_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Janus processes for this JANUS_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.janus/checkpoints/.  Opt-in via
        ...
        self._history_file = _janus_home / ".janus_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from janus_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Janus'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from janus_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from janus_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from janus_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from janus_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from janus_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Janus")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if runtime is None and _primary_exc is not None:
            from janus_cli.auth import AuthError
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: janus setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: janus setup")
            return False
        ...
        # `janus chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `janus auth add openai-codex`
        # without `janus model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from janus_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from janus_cli.models import resolve_fast_mode_overrides
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (janus sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (janus sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``janus doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``janus doctor`` so the banner stays
        small.
        """
        try:
            from janus_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Janus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from janus_cli.model_switch import is_nous_janus_non_agentic
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Janus: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Janus: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.janus/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from janus_cli.clipboard import save_clipboard_image

        img_dir = get_janus_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: janus --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Janus config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_janus_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from janus_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run janus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'janus setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from janus_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def _show_help_panel(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Janus!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in janus_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from janus_cli.main import _relative_time
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self.agent and self.conversation_history:
            # Trigger memory extraction on the old session before session_id
            # rotates. commit_memory_session also fires opt-in auto-mining
            # (agent/auto_mine.py) — the single cross-surface hook.
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("JANUS_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from janus_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `janus gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `janus sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from janus_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `janus sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.janus/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``janus --resume <id>``
        ...
        saved_dir = get_janus_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"janus_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: janus --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from janus_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `janus model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from janus_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from janus_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from janus_cli.model_switch import switch_model, parse_model_flags
        from janus_cli.providers import get_label
        ...
        if force_refresh:
            try:
                from janus_cli.models import clear_provider_models_cache
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from janus_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to janus_cli.curator so the CLI and the `janus curator`
        ...
        try:
            from janus_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from janus_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to janus_cli.skills_hub."""
        from janus_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_janus_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_janus_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in janus_cli/commands.py instead of touching every dispatch site.
        from janus_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from janus_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from janus_cli.skin_engine import get_active_skin
            ...
            from janus_cli.main import _print_version_info
            ...
            from janus_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from janus_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_janus_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from janus_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``janus bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: janus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `janus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Janus-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from janus_cli.goals import GoalManager
            from janus_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Janus keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from janus_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_janus_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from janus_cli.config import load_config
        from janus_cli.colors import Colors as _Colors
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``JANUS_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``JANUS_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Janus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Janus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Janus is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from janus_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from janus_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from janus_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Janus Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``janus update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from janus_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Janus Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Janus Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Janus Agent",
            detail="This will exit the current session and run `janus update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by janus_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors JANUS_DEV_CREDITS_FIXTURE
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from janus_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.janus/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_janus_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "janus_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "janus_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _janus_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard JANUS_HOME, so no -p needed.
            try:
                from janus_cli.profiles import get_active_profile_name
            ...
            print(f"  janus --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  janus -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from janus_cli.skin_engine import get_active_goodbye
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from janus_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Janus Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Janus Agent! Type your message or /help for commands."
        ...
        try:
            from janus_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("JANUS_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(JANUS_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Janus migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from janus_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in janus_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_janus_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Janus binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from janus_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Janus")
        ...
        # Dynamic prompt: shows Janus symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Janus needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Janus needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Janus needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = JanusCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_janus_osd_patched", False):
                ...
                _pt_renderer._janus_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``JANUS_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("JANUS_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the janus process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: janus setup"
            )
            _run_cleanup()
        ...
        if getattr(self, '_pending_relaunch', None):
            from janus_cli.relaunch import relaunch
...
def _run_kanban_goal_loop_q(cli: "JanusCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``JANUS_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("JANUS_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from janus_cli import kanban_db as _kb
    from janus_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Janus Agent CLI - Interactive AI Assistant
    ...
    try:
        from janus_cli.stdio import configure_windows_stdio
    ...
    if gateway:
        ...
        print("Starting Janus Gateway (messaging platforms)...")
    ...
    # Default to janus-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside JanusCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  JANUS_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("JANUS_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``janus chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("JANUS_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
                from janus_cli import kanban_db as _kb
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Janus" box, no tool-gen
                    ...
                    if os.environ.get("JANUS_KANBAN_GOAL_MODE") == "1":
                        try:
                    ...
                    if isinstance(result, dict) and result.get("failed"):
                        _exit_code = 1
                        if os.environ.get("JANUS_KANBAN_TASK") and result.get(
                            "failure_reason"
                        ) in ("rate_limit", "billing"):
                            try:
                                from janus_cli.kanban_db import (
            ...
            # Single-query mode (`janus chat -q "…"`): skip the welcome
