...
# Must match janus_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (janus-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``janus-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Cloud Industry Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. janus-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Cloud Industry Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.janus/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url:
        ...
        from janus_cli.runtime_provider import _detect_api_mode_for_url
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'janus auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (janus_cli/config.py load_config()) so that
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "acp_command": {
                "type": "string",
                "description": (
                    "Override ACP command for child agents (e.g. 'copilot') — children use ACP "
                    "subprocess transport instead of the parent's. Do NOT set unless the user has "
                    "explicitly told you a specific ACP-compatible CLI (e.g. GitHub Copilot CLI) is "
                    "installed and configured. Empty = parent's default transport (Janus subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Arguments for acp_command (default: ['--acp', '--stdio']). Only used when acp_command is set.",
            },
        },
        "required": [],
    },
