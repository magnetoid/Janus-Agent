"""
Configuration management for Janus Agent.

Config files are stored in ~/.janus/ for easy access:
- ~/.janus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.janus/.env         - API keys and secrets

This module provides:
- janus config          - Show current configuration
- janus config edit     - Open config in editor
- janus config set      - Set a specific value
- janus config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``janus config set`` (which rewrites
    ...
    ``config.yaml`` in place: janus never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.janus/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``janus logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  janus config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Janus makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Janus itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``janus update``, the TUI build.
...
#   on every plugin install / ``janus update``.
...
# * ``JANUS_HOME`` / ``JANUS_PROFILE`` / ``JANUS_CONFIG`` /
#   ``JANUS_ENV`` — Janus runtime location flags. Writing these into
...
#
# IMPORTANT: ``JANUS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (JANUS_GEMINI_CLIENT_ID,
# JANUS_LANGFUSE_PUBLIC_KEY, JANUS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Janus runtime location — never via dashboard env writer.
    # NOT a JANUS_* blanket: integration credentials (JANUS_GEMINI_*,
    # JANUS_LANGFUSE_*, JANUS_SPOTIFY_*, ...) ARE allowed.
    "JANUS_HOME", "JANUS_PROFILE", "JANUS_CONFIG", "JANUS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Janus runtime location "
            "(JANUS_HOME, JANUS_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `janus plugins enable
    # observability/langfuse` or `janus tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "JANUS_LANGFUSE_ENV",
    "JANUS_LANGFUSE_RELEASE",
    "JANUS_LANGFUSE_SAMPLE_RATE",
    "JANUS_LANGFUSE_MAX_CHARS",
    "JANUS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from janus_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("JANUS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_janus_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Janus is running in package-manager-managed mode.

    Two signals: the JANUS_MANAGED env var (set by the systemd service),
    or a .managed marker file in JANUS_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade janus-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Janus was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.janus/.install_method`` file (written by installers)
    2. JANUS_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``imbalabs/janus-agent`` image stamps ``docker``
    ...
    so ``janus update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_janus_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.janus/.install_method."""
    stamp = get_janus_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Janus lives in a ``uv tool`` layout.

    ``uv tool install janus-agent`` places the install at
    ``.../uv/tools/janus-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``janus-agent`` happens to be uv-tool-installed on the machine while
    the *active* Janus is a regular pip/venv install, causing
    ``janus update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/janus-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade janus-agent"
    if method == "docker":
        return "docker pull imbalabs/janus-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade janus-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade janus-agent"
        return "pip install --upgrade janus-agent"
    return "janus update"
...
# Long-form text for ``janus update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# janus_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Janus, it doesn't update
...
✗ ``janus update`` doesn't apply inside the Docker container.

Janus Agent runs as a published image (imbalabs/janus-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull imbalabs/janus-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate janus-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm imbalabs/janus-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/imbalabs/janus-agent/tags
  • Your config and session history live under ``$JANUS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``janus update`` inside Docker.
...
def format_managed_message(action: str = "modify this Janus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("JANUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Janus installation is managed by NixOS "
            f"(JANUS_MANAGED={env_hint}).\n"
            "Edit services.janus-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Janus installation is managed by Homebrew "
            f"(JANUS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade janus-agent"
        )

    return (
        f"Cannot {action}: this Janus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Janus."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from JANUS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, janus_bin
    or None if container mode is not active, we're already inside the
    container, or JANUS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("JANUS_DEV") == "1":
        return None
    ...
    container_mode_file = get_janus_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "janus-agent")
    exec_user = info.get("exec_user", "janus")
    janus_bin = info.get("janus_bin", "/data/current-package/bin/janus")
...
# Re-export from janus_constants — canonical definition lives there.
from janus_constants import get_janus_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_janus_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_janus_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_janus_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the JANUS_UID / JANUS_GID env vars set by Docker deployments.

    Docker containers running Janus commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level JANUS_HOME once, but
    subdirectories created at runtime by ``ensure_janus_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("JANUS_UID", "").strip()
    gid_str = os.environ.get("JANUS_GID", "").strip()
    try:
...
def _chown_to_janus_uid(path) -> None:
    """Chown ``path`` to ``JANUS_UID:JANUS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_janus_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_janus_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the janus group can
    share state with the gateway service.

    The mode can be overridden via the JANUS_HOME_MODE environment variable
    (e.g. JANUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse JANUS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``JANUS_UID``/``JANUS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("JANUS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_janus_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Janus runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("JANUS_CONTAINER") or os.environ.get("JANUS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set JANUS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into JANUS_HOME if the user doesn't have one yet."""
...
def ensure_janus_home():
    """Ensure ~/.janus directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_janus_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_janus_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"JANUS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Janus-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Janus
        ...
        # identity slot (SOUL.md). Empty by default. The JANUS_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Janus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Janus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Janus
        ...
        #  "/home/user/.janus/cache/documents:/output"]
        ...
        # bundled Janus image, which drops to the `janus` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Janus sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Janus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``janus chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.janus/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: janus sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Janus truncates the
    ...
    "compression": {
        ...
        "auto_threshold": 0.0,        # proactive compaction trigger as a fraction
                                      # of context_length; 0 disables the proactive
                                      # path (existing behavior). When > 0 the
                                      # compressor will compact on the next turn
                                      # boundary (or before the next LLM call)
                                      # once prompt_tokens cross this ratio even
                                      # if the user is idle / no new user input
                                      # is pending. Useful as:
                                      #   * an EARLIER trigger (e.g. 0.30) when
                                      #     you want more aggressive rolling
                                      #     compaction than the 50% default
                                      #   * a HARD CAP (e.g. 0.85) that
                                      #     guarantees compaction well before
                                      #     any provider-side limit is hit,
                                      #     complementing the post-error
                                      #     recovery path
                                      # `should_compress` returns True when
                                      # EITHER `threshold` or `auto_threshold`
                                      # is exceeded, so the more aggressive
                                      # value wins.
    ...
    "auxiliary": {
        ...
        # Invoked by ``janus kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``janus kanban decompose`` and the kanban
        ...
        # ``janus profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `janus model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `janus` (and `janus chat`) launches by default:
        ...
        # REPL and `--tui` (or JANUS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `janus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `janus -c` muscle memory.  Default off so existing
        # users aren't surprised.  JANUS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `janus --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/janus`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``JANUS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``JANUS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``JANUS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/janus``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.janus/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.janus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Janus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Janus auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `janus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `janus update`, so pruning them only
        ...
        # skipped), snapshot ~/.janus/skills/ into
        # ~/.janus/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `janus curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for janus-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Janus Agent*" header.
    ...
    "approvals": {
        ...
        "timeout": 60,
        "cron_mode": "deny",
        # How to handle dangerous commands and execute_code when running
        # outside an interactive TTY (is_cli=False) and outside a gateway
        # session (is_gateway=False) — e.g. background automation, local scripts,
        # or ACP headless servers.
        #   "deny"            (default) - safely blocks dangerous commands
        #   "approve"         - preserves legacy behavior (auto-approves with a warning)
        #   "require_gateway" - pauses and routes the approval request to the operator's
        #                       configured gateway channel (e.g. mobile Telegram)
        "headless_mode": "deny",
        ...
        # false.  TUI has its own modal overlay (JANUS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.janus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or JANUS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `janus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``janus_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Janus to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `janus -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `janus` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `janus kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     janus-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Janus tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.janus/logs/.
    ...
    "model_catalog": {
        ...
        # next /model or `janus model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.janus/.env,
        ...
        # behavior: files must live under the Janus cache, under
        ...
        # user. Bridged to JANUS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Janus
        # cache (~/.janus/cache/{documents,images,audio,video,screenshots})
        ...
        # to JANUS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # JANUS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to JANUS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.janus/state.db.
    ...
    "sessions": {
        ...
        # default of ``janus sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.janus/sessions/session_{sid}.json`` on every turn
    ...
    # ``janus update`` behaviour.
    "updates": {
        # Run a full ``janus backup``-style zip of JANUS_HOME before every
        # ``janus update``.  Backups land in ``<JANUS_HOME>/backups/`` and
        # can be restored with ``janus import <path>``.  Off by default —
        # on large JANUS_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `janus update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<JANUS_HOME>/lsp/bin/`` on first use.
    ...
    # `janus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.janus/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.janus/.env (or your shell) and never in config.yaml.
            ...
            # ~/.janus/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'janus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Janus auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @janus:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. JANUS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #janus)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: janus-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'janus-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Janus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Janus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Janus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # JANUS_TOOL_PROGRESS and JANUS_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.janus.config`` entries, then checks
    ...
    try:
        ...
        # should never break `janus update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Janus versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``janus_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.janus/.env.
    ...
    interactive menu render (`janus tools`, `janus setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `janus tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Janus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.janus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.janus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.janus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.janus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Janus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Janus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.janus/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Janus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale JANUS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("JANUS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale JANUS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'janus doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  janus config edit     # Edit config file", Colors.DIM))
    print(color("  janus config set <key> <value>", Colors.DIM))
    print(color("  janus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: janus config set <key> <value>")
            print()
            print("Examples:")
            print("  janus config set model anthropic/claude-sonnet-4")
            print("  janus config set terminal.backend docker")
            print("  janus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'janus config migrate' to add them")
        ...
        print("  janus config           Show current configuration")
        print("  janus config edit      Open config in editor")
        print("  janus config set <key> <value>   Set a config value")
        print("  janus config check     Check for missing/outdated config")
        print("  janus config migrate   Update config with new options")
        print("  janus config path      Show config file path")
        print("  janus config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``janus config`` UI so users
