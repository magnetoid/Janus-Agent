"""
Gateway subcommand for janus CLI.

Handles: janus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_janus_home is imported lazily at call sites to avoid ImportError
# when janus_constants is cached from a pre-update version during `janus update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "janus CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``janus gateway status`` from falsely counting the ``janus`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``janus gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "janus_cli.main gateway",
        "janus_cli.main --profile",
        "janus_cli.main -p",
        "janus_cli/main.py gateway",
        "janus_cli/main.py --profile",
        "janus_cli/main.py -p",
        "janus gateway",
        ...
        # ``janus.exe --profile`` / ``janus.exe -p`` would also match
        # ``janus.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "janus.exe gateway",
        "janus-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_janus_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"janus_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. JANUS_HOME
        ...
        # JANUS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "janus_home=" in command_lc
            and f"janus_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``janus update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Janus profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Janus profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from janus_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "janus_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``janus update`` users
    # with no running gateway until they re-invoke ``janus gateway``
...
def _sync_janus_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``JANUS_HOME``.

    Under ``sudo``, ``JANUS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_janus_home` falls back to ``/root/.janus`` — the wrong
    profile. The unit file pins ``JANUS_HOME`` for the actual gateway
    ...
    unit_home = env.get("JANUS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("JANUS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["JANUS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}janus gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}janus gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from janus_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from janus_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `janus gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``janus gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from janus_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (JANUS_HOME-scoped).
...
from janus_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Janus Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``janus gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``JANUS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("JANUS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "janus-gateway"
SERVICE_DESCRIPTION = "Janus Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current JANUS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (JANUS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from janus_constants import get_default_janus_root

    home = get_janus_home().resolve()
    default = get_default_janus_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary JANUS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(janus_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when JANUS_HOME is a named profile.

    For ``~/.janus/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        janus_home: Optional explicit JANUS_HOME path. Defaults to the current
            ``get_janus_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from janus_constants import get_default_janus_root

    home = Path(janus_home or str(get_janus_home())).resolve()
    default = get_default_janus_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this JANUS_HOME.

    Default ``~/.janus`` returns ``janus-gateway`` (backward compatible).
    Profile ``~/.janus/profiles/coder`` returns ``janus-gateway-coder``.
    Any other JANUS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Janus installs that predate the
# janus-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (janus-gateway-*.service) and unrelated third-party
# "janus" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("janus.service",)
...
def _find_legacy_janus_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Janus gateway units.

    Detects unit files installed by older Janus versions that used a
    different service name (e.g. ``janus.service`` before the rename to
    ``janus-gateway.service``). When both a legacy unit and the current
    ``janus-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``janus-gateway-coder.service`` and unrelated third-party
      ``janus-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``janus.service`` running an unrelated
...
def has_legacy_janus_units() -> bool:
    """Return True when any legacy Janus gateway unit files exist."""
    return bool(_find_legacy_janus_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Janus gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_janus_units()
    if not legacy:
        return
    print_warning("Legacy Janus gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current janus-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    janus gateway migrate-legacy")


def remove_legacy_janus_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Janus gateway unit files.

    Iterates over whatever ``_find_legacy_janus_units()`` returns — which is
    ...
    legacy = _find_legacy_janus_units()
    if not legacy:
        print("No legacy Janus gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Janus gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: janus gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo janus gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    janus gateway uninstall")
    print_info("    sudo janus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Janus can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo janus gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo janus gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo janus gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Janus often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.janus`` → ``ai.janus.gateway.plist`` (backward compatible).
    Profile ``~/.janus/profiles/coder`` → ``ai.janus.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.janus.gateway-{suffix}" if suffix else "ai.janus.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.janus/janus-agent  -> /home/alice/.janus/janus-agent
      /opt/janus                 -> /opt/janus  (kept as-is)
...
def _janus_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current JANUS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_janus_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.janus                    → /home/alice/.janus
      /root/.janus/profiles/coder     → /home/alice/.janus/profiles/coder
      /opt/custom-janus               → /opt/custom-janus  (kept as-is)
    """
    current_janus = get_janus_home().resolve()
    current_default = (Path.home() / ".janus").resolve()
    target_default = Path(target_home_dir) / ".janus"

    # Default ~/.janus → remap to target user's default
    if current_janus == current_default:
        return str(target_default)

    # Profile or subdir of ~/.janus → preserve the relative structure
    try:
        relative = current_janus.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.janus) — keep as-is
        return str(current_janus)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    janus_home = get_janus_home()
    janus_node = janus_home / "node" / "bin"
    if _is_dir(janus_node):
        candidates.append(str(janus_node))
    janus_nm = janus_home / "node_modules" / ".bin"
    if _is_dir(janus_nm):
        candidates.append(str(janus_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m janus_cli.main``, so module
    ...
    dir, or a clone that ``janus update`` later relocates/removes — the path
    ...
    ``JANUS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if JANUS_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        janus_home = _janus_home_for_target_user(home_dir)
        profile_arg = _profile_arg(janus_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's JANUS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(janus_home) if janus_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m janus_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="JANUS_HOME={janus_home}"
Restart=always
RestartSec=5
...
"""
    ...
    janus_home = str(get_janus_home().resolve())
    profile_arg = _profile_arg(janus_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only JANUS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp JANUS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../janus_test`` path into
    # ``Environment="JANUS_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Janus install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo janus gateway uninstall --system")
    print_info("         janus gateway install")
    print_info("         janus gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("JANUS_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (janus.service from pre-rename installs)
    # before installing the new janus-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}janus gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}janus gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}janus gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_janus_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `janus gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `janus gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}janus gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}janus gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}janus gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}janus gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.janus.gateway-{suffix}" if suffix else "ai.janus.gateway"
...
# (the documented `nohup janus gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m janus_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active JANUS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "janus_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup janus gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from janus_constants import display_janus_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: janus gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup janus gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Janus install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  janus gateway status             # Check status")
    from janus_constants import display_janus_home as _dhh
...
def launchd_stop():
    ...
    # `janus gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Janus install")
    else:
        print("⚠ Service definition is stale relative to the current Janus install")
        print("  Run: janus gateway start")
    ...
    if deep:
        log_file = get_janus_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("JANUS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Janus gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'janus' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/janus/docker/entrypoint.sh before the Janus command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$JANUS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set JANUS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `janus gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set JANUS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `janus gateway restart` which already
    ...
    # `janus gateway start/restart` — leaving the gateway vulnerable to
    ...
    # JANUS_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("JANUS_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from janus_constants import get_janus_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Janus will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @janus:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. janus) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Janus delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Janus agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Janus will use (e.g., janus@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Janus connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: janus pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Janus will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``janus setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run janus under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.janus/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_janus_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from janus_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'janus pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: janus pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'janus gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from janus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from janus_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: janus pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'janus gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from janus_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Janus will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Janus will store the returned account_id/token in ~/.janus/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `janus gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_janus_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `janus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `janus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "JanusAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``janus_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from janus_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.janus/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.janus/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_janus_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: janus gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: janus gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: janus gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo janus gateway install --system"
                        )
                    print_info("  Or run in foreground:  janus gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: janus gateway run")
                print_info(
                    "  For persistence:   tmux new -s janus 'janus gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: janus gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup janus gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'janus gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`janus_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from janus_cli.service_manager import (
    ...
    if profile is None:
        ...
        # JANUS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``janus gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``janus gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``janus gateway run`` are unaffected.
      2. ``JANUS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``janus gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``JANUS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("JANUS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("JANUS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${JANUS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if JANUS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or JANUS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-janus service's
    # pattern in docker/s6-rc.d/main-janus/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: janus gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: janus gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s janus 'janus gateway run'"
                )
                print()
            ...
            print()
            print(
                "  janus gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s janus 'janus gateway run'         # persistent via tmux"
            )
            print(
                "  nohup janus gateway run > ~/.janus/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  janus profile create <name>     # creates the s6 service slot")
                print("  janus -p <name> gateway start   # bring it up via s6")
                print("  janus status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: janus gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: janus gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: janus gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  janus profile delete <name>     # tears down the s6 service slot")
                print("  janus -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `janus -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: janus gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_JANUS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `janus gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_JANUS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `janus gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: janus gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  janus gateway install")
                else:
                    print("To install as a service:")
                    print("  janus gateway install")
                    print("  sudo janus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  janus gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup janus gateway run > ~/.janus/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s janus 'janus gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  janus gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  janus gateway install  # Install as user service")
                    print(
                        "  sudo janus gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Janus gateway unit files from
        # pre-rename installs (e.g. janus.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_janus_units(interactive=not yes, dry_run=dry_run)
