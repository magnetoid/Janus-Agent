...
# Audio file extensions Janus recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Janus' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Janus-created Telegram private-chat
...
from janus_constants import get_default_janus_root, get_janus_dir, get_janus_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.janus/.env manually."
)
...
# Default location: {JANUS_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_janus_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; JanusAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_janus_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_janus_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_janus_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_janus_dir("cache/screenshots", "browser_screenshots")
_JANUS_HOME = get_janus_home()
_JANUS_ROOT = get_default_janus_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "JANUS_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "JANUS_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "JANUS_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "JANUS_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _JANUS_HOME / "image_cache",
    _JANUS_HOME / "audio_cache",
    _JANUS_HOME / "video_cache",
    _JANUS_HOME / "document_cache",
    _JANUS_HOME / "browser_screenshots",
    ...
    _JANUS_HOME / "cache" / "images",
    _JANUS_HOME / "cache" / "audio",
    _JANUS_HOME / "cache" / "videos",
    _JANUS_HOME / "cache" / "documents",
    _JANUS_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Janus profile and shared Janus root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for janus_root in (_JANUS_HOME, _JANUS_ROOT):
        denied.append(janus_root / ".env")
        denied.append(janus_root / "auth.json")
        denied.append(janus_root / "credentials")
        denied.append(janus_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Janus
    secrets (``~/.janus/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``JANUS_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Janus-managed cache, under an operator-allowlisted root
    (``JANUS_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.janus/.env,
    # ~/.janus/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?janus\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+janus[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("JANUS_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "JANUS_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "JANUS_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.janus/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from janus_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          JANUS_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          JANUS_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          JANUS_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("JANUS_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("JANUS_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("JANUS_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _janus_run_generation is set on
