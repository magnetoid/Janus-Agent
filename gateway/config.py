...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_janus_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.janus/config.yaml (primary user-facing config)
    3. ~/.janus/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_janus_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``janus config set gateway.streaming.*``
